...
The editor is Monaco — VS Code's own — so multi-cursor, sticky scroll, Markdown preview, and even huge log files all feel exactly right. **D2 diagrams** preview live beside their source (Alt+D) — rendered by *your own* `d2` on whichever machine owns the file, with pan/zoom, copy-as-image, and SVG/PNG export; nothing bundled, and the source never leaves its host. The terminals are real system shells, grouped by host, and any of them can slide into a dedicated **Sessions** workspace (or the full-window **F11** view) to sit beside your agents — without ever restarting the shell.
